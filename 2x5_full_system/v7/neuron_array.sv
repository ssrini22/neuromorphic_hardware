...
// Description: 2x5 array top module
// Revision: 1.1
...
module neuron_array#(
    ...
    //ocalparam signed [1:0] DOWN = -2'sd1;
        ...
        for (i = 0; i < N_inputs; i++) begin : gen_inputs
            for (j = 0; j < N_neurons; j++) begin : gen_outputs
                synapse_unit_2 #(
                    ...
                    .init_weight(synapse_weights[N_neurons*i+j]),
                    .weight_to_accum(syn_out[N_neurons*i+j])
                );
            end
        end
    endgenerate
    ...
    //logic signed [b_weight+1:0] syn_col [N_inputs];
