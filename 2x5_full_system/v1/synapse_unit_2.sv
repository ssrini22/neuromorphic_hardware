...
//        (abs_delta_t <= LEARN); //add logic for fire, using neuron time is 0 now
    ...
    //assign pass_weight = ((weight_reg + stdp_lut[lut_index]) == 0) ? init_weight : ((abs_delta_t <= LEARN) ? (weight_reg + stdp_lut[lut_index]) : weight_reg); //add overflow log
    assign pass_weight = (($signed(weight_reg) + $signed(stdp_lut[lut_index])) == 0) ? 1 : ($signed(weight_reg) + $signed(stdp_lut[lut_index])); //add overflow log
